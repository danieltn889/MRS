...
const AIMatchBanner: React.FC<AIMatchBannerProps> = ({ matchFilter, onSetMatchFilter, aiMatchesCount, appliedJobsCount, filteredMatchesLength }) => {
  return (
    <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-lg p-4 mb-6 text-white">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          <div className="bg-white/20 p-3 rounded-full"><Zap className="w-6 h-6" /></div>
          <div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
              matchFilter === 'all' ? 'bg-white text-blue-600' : 'bg-white/20 text-white hover:bg-white/30'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
              matchFilter === 'applied' ? 'bg-white text-green-600' : 'bg-white/20 text-white hover:bg-white/30'
            }`}
          >
          ...
          {aiMatchesCount > 0 && (
            <>
              <button
                ...
                className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                  matchFilter === 'high' ? 'bg-white text-green-600' : 'bg-white/20 text-white hover:bg-white/30'
                }`}
              >
