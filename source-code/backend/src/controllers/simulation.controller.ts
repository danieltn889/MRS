...
class SimulationController extends BaseController {
  ...
  async updateTemplate(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const jobIdValue = updates.job_id || req.body.jobId || undefined;
      if (jobIdValue !== undefined) {
        if (jobIdValue) {
          const jobResult = await DatabaseService.query('SELECT id FROM jobs WHERE id = $1', [jobIdValue]);
          if (!jobResult.rows[0]) {
    ...
    `, [sessionId]);
      ...
      for (let i = 0; i < templateTasks.length; i++) {
        ...
        console.log(`\n🔍 [PUNCTUALITY] Task ${i}:`, {
          ...
          rawDurationMinutes: task.duration_minutes,
        ...
        console.log(`   Duration: ${taskDurationMinutes} min → ${taskTimeLimit} sec limit`);
        console.log(`   Weight: ${taskWeight}`);
        ...
        if (progress?.status === 'completed' && progress.completed_at) {
          ...
          console.log(`   Completed at: ${taskCompletedAt.toISOString()}`);
      ...
      console.log(`\n📊 PUNCTUALITY SUMMARY:`);
      console.log(`   Total weight: ${totalWeightSum}`);
      console.log(`   On-time weight: ${onTimeWeightSum}`);
      console.log(`   Partial credit: ${partialCreditSum.toFixed(2)}`);
      console.log(`   Total earned: ${(onTimeWeightSum + partialCreditSum).toFixed(2)}`);

      const punctualityScore = totalWeightSum > 0
        ? Math.round(((onTimeWeightSum + partialCreditSum) / totalWeightSum) * 100)
        : 0;

      console.log(`   FINAL SCORE: ${punctualityScore}%`);
